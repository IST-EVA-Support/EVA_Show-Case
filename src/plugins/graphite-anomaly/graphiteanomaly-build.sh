...
if [ -f "/opt/adlink/eva/plugins/libgraphiteanomaly.so" ]
then
    message_out "/opt/adlink/eva/plugins/libgraphiteanomaly.so exist, will remove it first."
    ${sudoString} rm /opt/adlink/eva/plugins/libgraphiteanomaly.so
    message_out "${sudoString} rm /opt/adlink/eva/plugins/libgraphiteanomaly.so is removed."
else
    message_out "/opt/adlink/eva/plugins/libgraphiteanomaly.so does not exist, will copy it"
fi

# copy built library to eva plugins folder
message_out "start copying to /opt/adlink/eva/plugins/ ......"
${sudoString} cp ./build/libgraphiteanomaly.so /opt/adlink/eva/plugins/.
...
message_out "Build plugin, graphiteanomaly, process completed!"
