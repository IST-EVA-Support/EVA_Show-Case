...
if %ModelNetwork% equ yolov3 (
	...
	if "%translatorplugin%"=="adtranslator" (
		...
		gst-launch-1.0 filesrc location=geo-fencing-demo.mp4 ! qtdemux ! h264parse ! avdec_h264 ! videoconvert ! adrt model=adlink-yolov3-geo-fencing.engine scale=0.004 mean="0 0 0" device=0 batch=1 rgbconv=true ! adtranslator label=adlink-yolov3-geo-fencing-label.txt topology=yolov3 dims=1,27,16,16,1,27,32,32,1,27,64,64 input_width=512 engine-type=2 ! geofencebase alert-area-def=alert-def-area-geo.txt area-display=true person-display=true alert-type=geofence ! email_alert alert-type=geofence receiver-address=youremail@domain.com ! voice_alert alert-type=geofence ! videoconvert ! d3dvideosink
	)
)
